...
void ui_handle_event(SDL_Event *e, UI_State *ui, BoardState *board) {
  ...
  case SDL_EVENT_MOUSE_MOTION:
    ...
    bool over_hchanger  = cursor_in_rect(mx, my, &ui->hlight_changer.rect);
    ...
    bool over_any = over_board    || over_toggler   || over_fen_btn  ||
                    over_pgn_btn  || over_tchanger  || over_achanger ||
                    over_hchanger || over_reset_btn || over_undo_btn ||
                    over_redo_btn;
    ...
    ui->hlight_changer.hovered  = over_hchanger;
  ...
  case SDL_EVENT_MOUSE_BUTTON_UP:
    if (e->button.button == SDL_BUTTON_LEFT) {
      ...
      if (cursor_in_rect(mx, my, &ui->hlight_changer.rect)) {
        ui->hlight_changer.color_idx       = (ui->hlight_changer.color_idx + 1) % HIGHLIGHT_COLORS_COUNT;
        board->annotations.highlight_color = ui->hlight_changer.colors[ui->hlight_changer.color_idx];
      }
