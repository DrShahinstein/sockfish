...
bool is_castling_move(BoardState *board, Move move) {
  ...
  if (piece == 'K') required_right = kingside ? CASTLE_WK : CASTLE_WQ;
  else              required_right = kingside ? CASTLE_BK : CASTLE_BQ;
...
bool is_en_passant_capture(BoardState *board, Move move) {
  ...
  char piece   = board->board[fr][fc];
  bool invalid = (piece != 'p' && piece != 'P') || fc == tc || board->board[tr][tc] != 0;
